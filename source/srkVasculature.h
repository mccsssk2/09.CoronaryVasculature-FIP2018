...
A margin of 2 on each side of the model.
*/
...
#define Pin			100.0 * 133.33 // mm Hg.: Convert mmHg to Pa (=N/m^2) by multiplying by 133.3. 1 mmHg = 133.3 Pa/m^2
#define Pout_C		20.0 * 133.33   // mm Hg.
...
double	x_cood, y_cood, z_cood;
double 	theta1, theta2, theta;
	int assignedOrNot, onOff;
	int whichCase;
	double old_radius, old_resistanceSegment;
...
typedef struct {double total_perfusion, distanceToEndo; int count; } arr;
...
// total perfusion at leaves.
double totalPerfusionLeaves(node *node){
 if(node == NULL) 									  	return 0;
 if(node->left == NULL && node->right == NULL)  			return node->perfusion;
 else if(node->left == NULL && node->right != NULL)			return totalPerfusionLeaves(node->right);
 else if(node->left != NULL && node->right == NULL)		        return totalPerfusionLeaves(node->left);
 else   												return totalPerfusionLeaves(node->left)+totalPerfusionLeaves(node->right);
}
...
x1 = x2 = 0.0; // at the time of doing simulations.
...
/* Helper function that allocates a new node with the
  ...
  temp->data 	= data;
	...
	temp->strahler = -1; //  viz_strahler;
	temp->parent_srahler = -1; //  viz_parent_strahler;
	temp->radius = -1; //  viz_radius;
	temp->length = -1; //  viz_length;
	temp->perfusion = -1; //  viz_perfusion;
	temp->pressure = -1; //  viz_pressure;
	temp->resistance = -1; //  viz_resistance;
	temp->resistanceSegment = -1; //  viz_resistanceSegment;
	temp->downResistance = -1; //  viz_downResistance;
	temp->x_cood = -1; //  viz_xc;
	temp->y_cood = -1; //  viz_yc;
	temp->z_cood = -1; //  viz_zc;
	temp->theta1 = -1; //  viz_theta1;
	temp->theta2 = -1; //  viz_theta2;
	temp->theta = -1; //  viz_theta;
	temp->assignedOrNot = -1; //  viz_assignedOrNot;
	temp->onOff = -1; //  viz_onOff;
	temp->whichCase = -1; //  viz_whichCase;
...
node *buildTreeR(int in[], int pre[], int inStrt, int inEnd, int oneOrTwo){
...
//  exit(0);
  ...
  tNode->left 		= buildTreeR(in, pre, inStrt, inIndex-1, oneOrTwo);
  tNode->right 	= buildTreeR(in, pre, inIndex+1, inEnd, oneOrTwo);
...
void writeTreeSoFar2(node *tNode, int fileNum){
...
// write inlet pressure and perfusion and radius. write stenosed node pressure and perfusion and radius. these are 3 separate files.
// stenosed node data.
if( (tNode!=NULL) && (tNode->whichCase==2) && (tNode->strahler==11) && (fabs(tNode->x_cood - 60.222874)<0.01 ) && (fabs(tNode->y_cood - 36.000000) < 0.01) && (fabs(tNode->z_cood - 25.734643)<0.01) ){
	sprintf(str, "StenosedNodeData%d.data", fileNum);
	output = fopen(str,"a+");
	fprintf(output, "%50.50lf %50.50lf %50.50lf %d %d %50.50lf %50.50lf %50.50lf %50.50lf %d %d %50.50lf\n", tNode->x_cood, tNode->y_cood, tNode->z_cood, tNode->data, tNode->parent_data, tNode->radius, tNode->resistanceSegment, tNode->pressure, tNode->perfusion, tNode->strahler, tNode->parent_srahler, tNode->downResistance ); 	
	fclose(output);
}

// LA root data.
if( (tNode!=NULL) && (tNode->whichCase==2||tNode->whichCase==3) && (tNode->parent_data==-1) ){
	sprintf(str, "LAROOTDATA%d.data", fileNum);
	output = fopen(str,"a+");
	fprintf(output, "%50.50lf %50.50lf %50.50lf %d %d %50.50lf %50.50lf %50.50lf %50.50lf %d %d %50.50lf\n", tNode->x_cood, tNode->y_cood, tNode->z_cood, tNode->data, tNode->parent_data, tNode->radius, tNode->resistanceSegment, tNode->pressure, tNode->perfusion, tNode->strahler, tNode->parent_srahler, tNode->downResistance ); 	
	fclose(output);
}
...
// RCA root data.
if( (tNode!=NULL) && (tNode->whichCase==1) && (tNode->parent_data==-1) ){
	sprintf(str, "RCAROOTDATA%d.data", fileNum);
	output = fopen(str,"a+");
	fprintf(output, "%50.50lf %50.50lf %50.50lf %d %d %50.50lf %50.50lf %50.50lf %50.50lf %d %d %50.50lf\n", tNode->x_cood, tNode->y_cood, tNode->z_cood, tNode->data, tNode->parent_data, tNode->radius, tNode->resistanceSegment, tNode->pressure, tNode->perfusion, tNode->strahler, tNode->parent_srahler, tNode->downResistance ); 	
	fclose(output);
}
...
// go to left and right only if me is already assigned.
if(tNode->left!=NULL) writeTreeSoFar2(tNode->left, fileNum);
if(tNode->right!=NULL) writeTreeSoFar2(tNode->right, fileNum);
...
output = fopen("ALL_1/NodeData.data.1","r");
...
void nodesThatDidntRead(node *tNode){

if(tNode->whichCase < 1)
printf("%d %d\n", tNode->data, tNode->whichCase);

if(tNode->left!=NULL) nodesThatDidntRead(tNode->left);
if(tNode->right!=NULL) nodesThatDidntRead(tNode->right);

 }
 
 void constructTree(node *tNode, int oneOrTwo){
...
// read in node data.
// sprintf(str, "ALL_%d/NodeData.data.%d", fileNum, oneOrTwo);
sprintf(str, "NodeData.data.%d", oneOrTwo); // fileNum is now set to 0.
output = fopen(str, "r");
...
//     fprintf(out, "%d %d %d %d %50.50f %50.50f %50.50f %50.50f %50.50f %50.50f %50.50f %50.50f %50.50f %50.50f %50.50f %50.50f %50.50f %d %d %d\n", node->data, node->parent_data, node->strahler, node->parent_srahler, node->radius, node->length, node->perfusion, node->pressure, node->resistance, node->resistanceSegment, node->downResistance, node->x_cood, node->y_cood, node->z_cood, node->theta1, node->theta2, node->theta, node->assignedOrNot, node->onOff, node->whichCase);	
while( fscanf(output, "%d %d %d %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %d %d %d\n", &viz_data, &viz_parent_data, &viz_strahler, &viz_parent_strahler, &viz_radius, &viz_length, &viz_perfusion, &viz_pressure, &viz_resistance, &viz_resistanceSegment, &viz_downResistance, &viz_xc, &viz_yc, &viz_zc, &viz_theta1, &viz_theta2, &viz_theta, &viz_assignedOrNot, &viz_onOff, &viz_whichCase)!=EOF  ){
if(tNode->data==viz_data){
	tNode->parent_data = viz_parent_data;
	tNode->strahler = viz_strahler;
	tNode->parent_srahler = viz_parent_strahler;
	tNode->radius = viz_radius;
	tNode->length = viz_length;
	tNode->perfusion = viz_perfusion;
	tNode->pressure = viz_pressure;
	tNode->resistance = viz_resistance;
	tNode->resistanceSegment = viz_resistanceSegment;
	tNode->downResistance = viz_downResistance;
	tNode->x_cood = viz_xc;
	tNode->y_cood = viz_yc;
	tNode->z_cood = viz_zc;
	tNode->theta1 = viz_theta1;
	tNode->theta2 = viz_theta2;
	tNode->theta = viz_theta;
	tNode->assignedOrNot = viz_assignedOrNot;
	tNode->onOff = viz_onOff;
	tNode->whichCase = viz_whichCase;
	
//	printf("read in data: %d \n", tNode->data);
	
	break; // save a few micro-seconds.
}
};


} // end of if else read in NodeData.data.1
fclose(output); // this is the nodeData read in.

if(tNode->left!=NULL) constructTree(tNode->left, oneOrTwo);
if(tNode->right!=NULL) constructTree(tNode->right, oneOrTwo);

 } // constructTree.


// this didnt go so much. So I am going to break it down.
void writeAtLeaf(node *tNode, int fileNum, arr ***res1, double dx, double ***res2, double dx2, double ***res3, double dx3, double *res1PerfusionCountRV, double *res1PerfusionCountLV, int ***ires2, int ***ires3){
...
int dist_int;
 ...
 if (tNode->left == NULL && tNode->right == NULL  /* && (tNode->strahler==6) */  ){ // these are O(6) nodes by definition.
 // write all of it.
sprintf(str,"forHistogram%d.data", fileNum); // fileNum is now set to 0.
output = fopen(str,"a+");
fprintf(output, "%20.20f %d %f %f %f %f %f %f %f\n", tNode->perfusion, tNode->strahler, tNode->pressure, tNode->radius, tNode->length, tNode->resistanceSegment, tNode->x_cood, tNode->y_cood, tNode->z_cood);
fclose(output);

	usr_i = (int)((tNode->x_cood)/dx); usr_j = (int)((tNode->y_cood)/dx); usr_k = (int)((tNode->z_cood)/dx);
	
// the tNode->perfusion < 5 seems to be working.
// res1. 
//	printf("pre: %d %d %d %20.20f %20.20f\n", usr_i, usr_j, usr_k, res1[usr_k][usr_j][usr_i].total_perfusion, tNode->perfusion );
	res1[usr_k][usr_j][usr_i].total_perfusion = res1[usr_k][usr_j][usr_i].total_perfusion + tNode->perfusion;

	if(tNode->perfusion>50.0 && debug)
	printf("post: %d %d %d %20.20f %20.20f %d\n", usr_i, usr_j, usr_k, res1[usr_k][usr_j][usr_i].total_perfusion, tNode->perfusion , tNode->whichCase ); 
	
		// sum of perfusion in each of the layers.
		if(tNode->whichCase==1 ){ // RCA
			dist_int = 0;
			dist_int = (int) ( floor(res1[usr_k][usr_j][usr_i].distanceToEndo) );
			if(dist_int < 0) exit(0);
if(dist_int<6)
			res1PerfusionCountRV[dist_int] = res1PerfusionCountRV[dist_int] + tNode->perfusion;
		}

		if( (tNode->whichCase==2 || tNode->whichCase==3)  ){ // LA
			dist_int = 0;
			dist_int = (int) ( floor(res1[usr_k][usr_j][usr_i].distanceToEndo) );
			if(dist_int < 0) exit(0);
if(dist_int<12)
			res1PerfusionCountLV[dist_int] = res1PerfusionCountLV[dist_int] + tNode->perfusion;
		}
	
	// res2. res2 is assumed to be the higher resolution, i.e. smaller dx.
// if(tNode->strahler==6  ){ // most "small" perfusion is less than 5 AU
	usr_i = (int)floor(((tNode->x_cood)/dx2 )); usr_j = (int)floor(((tNode->y_cood)/dx2)); usr_k = (int)floor(((tNode->z_cood)/dx2));
mysize_x2=(int)((double)TOTAL_X/(double)dx2);  mysize_y2=(int)((double)TOTAL_Y/(double)dx2); mysize_z2=(int)((double)TOTAL_Z/(double)dx2);	

if(usr_i>mysize_x2-1 || usr_j>mysize_y2-1 || usr_k>mysize_z2-1){
	printf("wrong dx2. exit.\n"); exit(0);
}
	res2[usr_k][usr_j][usr_i] = res2[usr_k][usr_j][usr_i] + tNode->perfusion;
	if(tNode->perfusion>0) ires2[usr_k][usr_j][usr_i]++;
	
	usr_i = (int)floor(((tNode->x_cood)/dx3)); usr_j = (int)floor(((tNode->y_cood)/dx3)); usr_k = (int)floor(((tNode->z_cood)/dx3));
mysize_x3=(int)((double)TOTAL_X/(double)dx3);  mysize_y3=(int)((double)TOTAL_Y/(double)dx3); mysize_z3=(int)((double)TOTAL_Z/(double)dx3);
if(usr_i>mysize_x3-1 || usr_j>mysize_y3-1 || usr_k>mysize_z3-1){
	printf("wrong dx3. exit.\n"); exit(0);
}
	res3[usr_k][usr_j][usr_i] = res3[usr_k][usr_j][usr_i] + tNode->perfusion;
	if(tNode->perfusion>0) ires3[usr_k][usr_j][usr_i]++;	
// }

 }

 else  if(tNode->left == NULL && tNode->right != NULL){ writeAtLeaf(tNode->right, fileNum, res1, dx, res2, dx2, res3, dx3, res1PerfusionCountRV, res1PerfusionCountLV, ires2, ires3); }
 else  if(tNode->left != NULL && tNode->right == NULL){ writeAtLeaf(tNode->left, fileNum, res1, dx, res2, dx2, res3, dx3, res1PerfusionCountRV, res1PerfusionCountLV, ires2, ires3); }
 else  if(tNode->left != NULL && tNode->right != NULL){ 
	 writeAtLeaf(tNode->left, fileNum, res1, dx, res2, dx2, res3, dx3, res1PerfusionCountRV, res1PerfusionCountLV, ires2, ires3); 
	 writeAtLeaf(tNode->right, fileNum, res1, dx, res2, dx2, res3, dx3, res1PerfusionCountRV, res1PerfusionCountLV, ires2, ires3); 
 }

} // writeAtLeaf function.


void stenoseAllArteriesInThisOrder2(node *tNode, int fileNum){
	
if(tNode==NULL) return;

if( (tNode!=NULL) && (tNode->strahler==6) ){

	tNode->old_radius = tNode->radius;
	tNode->old_resistanceSegment = tNode->resistanceSegment;
	
	tNode->radius = ( (double)fileNum / 10.0 ) * tNode->radius; // run 1.

	// increase resistence, fileNum is between 0 and 10.
	tNode->resistanceSegment 	= mu_0 * (tNode->length)/ ( M_PI * (tNode->radius) * (tNode->radius) * (tNode->radius) * (tNode->radius) );
	return;
	}
	else if(tNode->left!=NULL && tNode->right!=NULL){ stenoseAllArteriesInThisOrder2(tNode->left, fileNum);  stenoseAllArteriesInThisOrder2(tNode->right, fileNum);  }
	else if(tNode->left!=NULL && tNode->right==NULL){ stenoseAllArteriesInThisOrder2(tNode->left, fileNum);   }
	else if(tNode->left==NULL && tNode->right!=NULL){ stenoseAllArteriesInThisOrder2(tNode->right, fileNum);  }	
	else if(tNode->left==NULL && tNode->right==NULL){  
		return;		
	}
}
...
void stenoseAllArteriesInThisOrder3(node *tNode, int fileNum){
	
if(tNode==NULL) return;

if( (tNode!=NULL) && (tNode->strahler==10) && (tNode->parent_srahler==11) ){
	...
	// increase resistence, fileNum is between 0 and 10.
	tNode->resistanceSegment 	= mu_0 * (tNode->length)/ ( M_PI * (tNode->radius) * (tNode->radius) * (tNode->radius) * (tNode->radius) );
//	printf("new: %f %d\n", tNode->resistanceSegment, fileNum);
	return;
	}
	else if(tNode->left!=NULL && tNode->right!=NULL){ stenoseAllArteriesInThisOrder3(tNode->left, fileNum);  stenoseAllArteriesInThisOrder3(tNode->right, fileNum);  }
	else if(tNode->left!=NULL && tNode->right==NULL){ stenoseAllArteriesInThisOrder3(tNode->left, fileNum);   }
	else if(tNode->left==NULL && tNode->right!=NULL){ stenoseAllArteriesInThisOrder3(tNode->right, fileNum);  }	
	else if(tNode->left==NULL && tNode->right==NULL){  
		return;		
	}
}
...
void stenoseAllArteriesInThisOrder(node *tNode, int fileNum){
	
if(tNode==NULL) return;

if( (tNode!=NULL) && (tNode->strahler==fileNum)  && (tNode->parent_srahler==(fileNum+1) ) ){

	tNode->old_radius = tNode->radius;
	tNode->old_resistanceSegment = tNode->resistanceSegment;
	
			tNode->radius = 0.1 * tNode->radius; // run 1.
//			tNode->radius = 0.5 * tNode->radius; // run 2.
...
//	printf("new: %f %d\n", tNode->resistanceSegment, fileNum);
	...
	else if(tNode->left!=NULL && tNode->right!=NULL){ stenoseAllArteriesInThisOrder(tNode->left, fileNum);  stenoseAllArteriesInThisOrder(tNode->right, fileNum);  }
	else if(tNode->left!=NULL && tNode->right==NULL){ stenoseAllArteriesInThisOrder(tNode->left, fileNum);   }
	else if(tNode->left==NULL && tNode->right!=NULL){ stenoseAllArteriesInThisOrder(tNode->right, fileNum);  }	
	else if(tNode->left==NULL && tNode->right==NULL){  
		return;		
	}
}
...
void stenoseThisArtery(node *tNode, int fileNum){
	
if(tNode==NULL) return;

//   tNode->assignedOrNot = 1; tNode->x_cood = 60.222874; tNode->y_cood = 36.000000; tNode->z_cood = 25.734643;  tNode->onOff = 1; 	
if( (tNode!=NULL) && (tNode->whichCase==2) && (tNode->strahler==11) && (fabs(tNode->x_cood - 60.222874)<0.01 ) && (fabs(tNode->y_cood - 36.000000) < 0.01) && (fabs(tNode->z_cood - 25.734643)<0.001) ){
	...
	else if(tNode->left!=NULL && tNode->right!=NULL){ stenoseThisArtery(tNode->left, fileNum);  stenoseThisArtery(tNode->right, fileNum);  }
	else if(tNode->left!=NULL && tNode->right==NULL){ stenoseThisArtery(tNode->left, fileNum);   }
	else if(tNode->left==NULL && tNode->right!=NULL){ stenoseThisArtery(tNode->right, fileNum);  }	
	else if(tNode->left==NULL && tNode->right==NULL){  
		return;		
	}
}

void reviseTerminals(node *tNode){

// call the relevant trees
	if(tNode->left!=NULL && tNode->right!=NULL){ reviseTerminals(tNode->left);  reviseTerminals(tNode->right);  }
else if(tNode->left!=NULL && tNode->right==NULL){ reviseTerminals(tNode->left);   }
else if(tNode->left==NULL && tNode->right!=NULL){ reviseTerminals(tNode->right);  }	
else if(tNode->left==NULL && tNode->right==NULL){  // this is a terminal, revise ALL terminals to what you want.

// making all terminals at same radius and length helped, but I want some realness to it.
// if this test does not work, go back to ALL terminals at radius 0.066 and length 1.5

//	if(tNode->strahler>6){
			tNode->strahler = 6;
			tNode->radius = 0.066; // hard coded value, unique value much smaller than any other radii.
			tNode->length = 1.5; // hard coded value, unique value much smaller than any other radii.
//		}

		tNode->resistanceSegment 	= 8.0 * mu_0 * (tNode->length)/ ( M_PI * (tNode->radius) * (tNode->radius) * (tNode->radius) * (tNode->radius) );
	}
	if(debug){
		// this perfusion as well as pressure is old - this is not to be used in calculations.
		if(tNode->strahler>6) 		printf("This bloody node is too big an order, %d %f %f\n", tNode->strahler, tNode->radius, tNode->perfusion);
		else 					printf("This bloody node is okay in terms of order!, %d %f %f\n", tNode->strahler, tNode->radius, tNode->perfusion);
	}	
	return; 
} // end of reviseTerminals.


/* This function works, but did not give desirable result. I need to experiment with this more.
void IncreaseAnisotropyRatio(node *tNode, int fileNum){
	
	if(tNode->left!=NULL && tNode->right!=NULL)	{
		if(tNode->strahler<=8){ // only transmural anisotropy is increased because of my baseline anistropy.
//			tNode->left->radius 	= ((double)(fileNum))/10.0 * tNode->left->radius;
			tNode->right->radius 	= ((double)(fileNum))/10.0 * tNode->right->radius;
			tNode->right->resistanceSegment 	= 8.0 * mu_0 * (tNode->right->length)/ ( M_PI * (tNode->right->radius) * (tNode->right->radius) * (tNode->right->radius) * (tNode->right->radius) );
		}
		IncreaseAnisotropyRatio(tNode->left, fileNum); 	IncreaseAnisotropyRatio(tNode->right, fileNum);		
	}
else if(tNode->left!=NULL && tNode->right==NULL)	 {  
			tNode->left->radius 	= tNode->radius;
 tNode->left->resistanceSegment 	= 8.0 * mu_0 * (tNode->left->length)/ ( M_PI * (tNode->left->radius) * (tNode->left->radius) * (tNode->left->radius) * (tNode->left->radius) );			
	IncreaseAnisotropyRatio(tNode->left, fileNum);  }
else if(tNode->left==NULL && tNode->right!=NULL)  {  
			tNode->right->radius 	= tNode->radius;
tNode->right->resistanceSegment 	= 8.0 * mu_0 * (tNode->right->length)/ ( M_PI * (tNode->right->radius) * (tNode->right->radius) * (tNode->right->radius) * (tNode->right->radius) );	
			IncreaseAnisotropyRatio(tNode->right, fileNum);  }	
else if(tNode->left==NULL && tNode->right==NULL){ return; }

} // end of IncreaseAniso ratio. function.
*/

/* blocking just O(6) left or right didnt really work: revise this function
void IncreaseAnisotropyRatio(node *tNode, int fileNum){
...
if(fileNum==1)            fraction = 1.1;
else if(fileNum==2)   fraction = 1.25;
else if(fileNum==3)   fraction = 1.5;

if(tNode->left!=NULL && tNode->right!=NULL){
		fraction2 						= tNode->left->radius / tNode->radius;
		tNode->left->radius 				= tNode->radius;	
		tNode->left->resistanceSegment 	= pow(fraction2, 4.0)    * tNode->left->resistanceSegment    ;		

		tNode->right->radius 				= (1.0/fraction) * tNode->right->radius;
		tNode->right->resistanceSegment 	= pow(fraction, 4.0) * tNode->right->resistanceSegment ;
		IncreaseAnisotropyRatio(tNode->left, fileNum);  IncreaseAnisotropyRatio(tNode->right, fileNum);
	}
else if(tNode->left!=NULL && tNode->right==NULL)	{ 
		fraction2 						= tNode->left->radius / tNode->radius;
...
void writeAnisotropicRatios(node *tNode, int fileNum){ // used in vascExpts.c

FILE *output;
double q0, qL, qR, q_bigger, q_smaller;
double gammaL, gammaR, gamma_big, gamma_small;
int SNp, SNL, SNR;
char str[1000];

 	sprintf(str, "anisotropy%d.data", fileNum);

	if(tNode->left!=NULL && tNode->right!=NULL){   
		SNp = tNode->strahler;
		SNL = tNode->left->strahler;
		SNR = tNode->right->strahler;
		
		q0 = tNode->perfusion;
		qL =  tNode->left->perfusion;
		qR = tNode->right->perfusion; 
		gammaL = qL / q0;
		gammaR = qR / q0;
		
		if(gammaL > gammaR)	{ gamma_big = gammaL; gamma_small = gammaR; }
		else 					{ gamma_big = gammaR; gamma_small = gammaL; }
		
		output = fopen(str,"a+");
		fprintf(output,"%d %d %d %f %f %f %10.10f %10.10f %10.10f %10.10f\n", SNp, SNL, SNR, q0, qL, qR, gammaL, gammaR, gamma_big, gamma_small);
		fclose(output);
		
		writeAnisotropicRatios(tNode->left, fileNum); writeAnisotropicRatios(tNode->right, fileNum);  
	}
else if(tNode->left!=NULL && tNode->right==NULL){  

		SNp = tNode->strahler;
		SNL = tNode->left->strahler;
		SNR = -1; // tNode->right->strahler;
		
		q0 = tNode->perfusion;
		qL =  tNode->left->perfusion;
		qR = -1; // tNode->right->perfusion; 
		gammaL = qL / q0;
		gammaR = -1; // qR / q0;
	...
	writeAnisotropicRatios(tNode->left, fileNum);  
	}
else if(tNode->left==NULL && tNode->right!=NULL){

		SNp = tNode->strahler;
		SNL = -1; // tNode->left->strahler;
		SNR = tNode->right->strahler;
		
		q0 = tNode->perfusion;
		qL =  -1; // tNode->left->perfusion;
		qR = tNode->right->perfusion; 
		gammaL = qL / q0;
		gammaR = qR / q0;
	...
	writeAnisotropicRatios(tNode->right, fileNum);  
	}
else if(tNode->left==NULL && tNode->right==NULL){ 
...
} // end of writeAniso ratios function.


void reviseTreeData(node *tNode, double fraction){
	
double scaleRadius, scaleResistance;

// scaleRadius = 1.0; scaleResistance = 1.0; 

if(tNode==NULL) return;

//		scaleRadius = fraction;		
		scaleResistance = 1.0; 	

//		printf("scaleRadius: %f\n", scaleRadius);
...
// call the relevant trees
if(tNode->left!=NULL && tNode->right!=NULL){ reviseTreeData(tNode->left, fraction);  reviseTreeData(tNode->right, fraction);  }
else
if(tNode->left!=NULL && tNode->right==NULL){ reviseTreeData(tNode->left, fraction);   }
else
if(tNode->left==NULL && tNode->right!=NULL){ reviseTreeData(tNode->right, fraction);  }	
else
if(tNode->left==NULL && tNode->right==NULL){ 
	
if(genrand_real1() < fraction && tNode->strahler==6){
	tNode->radius = 0.1 * ( tNode->radius );
	tNode->resistanceSegment = scaleResistance * (8.0*mu_0*(tNode->length)/(M_PI*(tNode->radius)*(tNode->radius)*(tNode->radius)*(tNode->radius) ) );	
}
else{
	tNode->radius = ( tNode->radius );
	tNode->resistanceSegment = scaleResistance * (8.0*mu_0*(tNode->length)/(M_PI*(tNode->radius)*(tNode->radius)*(tNode->radius)*(tNode->radius) ) );		
}
	return;  }	
...
void restoreTreeData(node *tNode){

if(tNode==NULL) return;

	tNode->radius = tNode->old_radius;
	tNode->resistanceSegment = tNode->old_resistanceSegment;

// call the relevant trees
if(tNode->left!=NULL && tNode->right!=NULL){ restoreTreeData(tNode->left);  restoreTreeData(tNode->right);  }
else
if(tNode->left!=NULL && tNode->right==NULL){ restoreTreeData(tNode->left);   }
else
if(tNode->left==NULL && tNode->right!=NULL){ restoreTreeData(tNode->right);  }	
else
if(tNode->left==NULL && tNode->right==NULL){ return;  }	
